...
export const LOCAL_ONLY_API_PREFIXES: ReadonlyArray<string> = [
  ...
  "/api/v1/agent", // Agentic ReAct loop: the model can emit eval_code / execute_command tool calls, which run through src/lib/agentic/toolRegistry → src/lib/skills/builtins → sandboxRunner.run → childProcess.spawn("docker", …). The spawn is TRANSITIVE (route.ts does not import child_process directly, so the source-scan subcheck of check-route-guard-membership cannot see it), but the RCE-via-tunnel surface is identical to /api/vnc-session and /api/local/ (both spawn docker and are LOCAL_ONLY). Loopback enforcement runs before auth, so a leaked JWT over a tunnel cannot drive a host container spawn (Hard Rules #15 + #17).
];
